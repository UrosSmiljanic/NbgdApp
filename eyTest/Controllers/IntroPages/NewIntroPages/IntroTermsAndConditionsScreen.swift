//
//  IntroTermsAndConditionsScreen.swift
//  eyTest
//
...
class IntroTermsAndConditionsScreen: UIViewController {
    ...
    let introNavigation: UIImageView = {
        ...
        view.image = #imageLiteral(resourceName: "file")
        return view
    }()
        ...
        if defaultLanguage == "sr" {
            ...
            fetchGenericData(urlString: url) { (introData: IntroPagesSr?, err) in
                ...
                DispatchQueue.main.async {
                    self.introTextView.attributedText = formatHtmlString(htmlString: introData?.entity?.mobIntroTermsSr ?? "<html><body> </body></html>")
                    self.titleLabel.text = "Услови коришћења"
                    self.setupView()
                    self.removeSpinner()
                }
            }
            ...
            fetchGenericData(urlString: url) { (introData: IntroPagesModel?, err) in
                ...
                DispatchQueue.main.async {
                    self.introTextView.attributedText = formatHtmlString(htmlString: introData?.entity.mobIntroTermsEn ?? "<html><body> </body></html>")
