//
//  IntroPushNotificatrionsScreen.swift
//  eyTest
//
...
class IntroPushNotificatrionsScreen: UIViewController {
    ...
    let introNavigation: UIImageView = {
        ...
        view.image = #imageLiteral(resourceName: "bell")
        return view
    }()
    ...
    @objc func handleNextPage() {
        ...
        let nextVc  = IntroSettingsScreen()
        ...
        if defaultLanguage == "sr" {
            ...
            fetchGenericData(urlString: url) { (introData: IntroPagesSr?, err) in
                ...
                DispatchQueue.main.async {
                    self.introTextView.attributedText = formatHtmlString(htmlString: introData?.entity?.mobIntroPushSr ?? "<html><body> </body></html>")
                    self.titleLabel.text = "Push нотификације"
                    self.setupView()
                    self.removeSpinner()
                }
            }
            ...
            fetchGenericData(urlString: url) { (introData: IntroPagesModel?, err) in
                ...
                DispatchQueue.main.async {
                    self.introTextView.attributedText = formatHtmlString(htmlString: introData?.entity.mobIntroPushEn ?? "<html><body> </body></html>")
